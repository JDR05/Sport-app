...
const emptySchedule: Schedule = { workPattern: null, freeSlots: [], commitments: [], wakeTimes: {} }
...
describe('basalRate', () => {
  ...
  it('treats unspecified sex as the higher of the two variants', () => {
    ...
    expect(unspecified).toBe(basalRate({ weightKg: 70, heightCm: 170, ageYears: 35, sex: 'male' }))
    expect(unspecified).toBeGreaterThan(
      basalRate({ weightKg: 70, heightCm: 170, ageYears: 35, sex: 'female' }),
    )
  })
})
...
describe('activityFactor', () => {
  it('rises with training volume', () => {
    expect(activityFactor(emptySchedule, 2)).toBeGreaterThan(activityFactor(emptySchedule, 0))
    expect(activityFactor(emptySchedule, 5)).toBeGreaterThan(activityFactor(emptySchedule, 2))
  })

  it('is lower for sedentary work at the same training volume', () => {
    expect(activityFactor({ ...emptySchedule, workPattern: 'office' }, 3)).toBeLessThan(
      activityFactor({ ...emptySchedule, workPattern: 'shift' }, 3),
    )
  })
})
...
describe('computeEnergy', () => {
  it('records an assumption for every missing input rather than failing', () => {
    const result = computeEnergy({
      profile: profileWith({ birthYear: null, heightCm: null, weightKg: null, sexAtBirth: null }),
