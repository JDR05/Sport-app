...
class FailingAdapter implements AiAdapter {
  ...
  async weeklyNote(): Promise<AiResult<never>> {
    return { ok: false, reason: this.reason, detail: 'simulated' }
  }
  async askQuestions(): Promise<AiResult<never>> {
    return { ok: false, reason: this.reason, detail: 'simulated' }
  }
  async ask(): Promise<AiResult<never>> {
    return { ok: false, reason: this.reason, detail: 'simulated' }
  }
  ...
  async dailyBrief(): Promise<AiResult<never>> {
    return { ok: false, reason: this.reason, detail: 'simulated' }
  }
}
