  ...
  return (
    <Framed>
        ...
        <textarea
          ...
          className="mt-2 w-full resize-none rounded-control border border-line bg-surface px-3 py-2.5 text-base leading-relaxed text-ink outline-none placeholder:text-faint focus:border-accent"
        />
      ...
      <div className="-mx-1 mt-2 flex snap-x gap-1.5 overflow-x-auto px-1 pb-1">
        {steps.map((n) => (
          <button
            ...
            onClick={() => onChange(value === n ? null : n)}
            className={`shrink-0 snap-start rounded-control border px-3.5 py-2 text-sm transition-[background-color,border-color] duration-[var(--motion-tap)] ${
              value === n
                ? 'border-accent bg-accent text-[color:var(--accent-ink)]'
                : 'border-line bg-surface text-muted'
            }`}
