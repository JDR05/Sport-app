...
export default async function LoginPage({ searchParams }: PageProps<'/login'>) {
  ...
  return (
    <Screen>
      ...
      {justConfirmed && (
        <p className="rounded-control bg-accent-soft px-3 py-2.5 text-sm text-ink">
          E-Mail bestätigt. Melde dich jetzt einmal an — danach bleibst du angemeldet.
        </p>
      )}

      {linkBroken && (
        <p role="alert" className="rounded-control bg-warn-soft px-3 py-2.5 text-sm text-ink">
